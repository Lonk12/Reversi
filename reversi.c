...
INT moves[61] = {11, 18, 81, 88, 13, 31, 16, 61, 38, 83, 68, 86, 14, 41, 15, 51, 48, 84, 58, 85, 33, 36, 63, 66, 34, 35, 43, 46, 53, 56, 64, 65, 24, 25, 42, 47, 52, 57, 74, 75, 23, 26, 32, 37, 62, 67, 73, 76, 12, 17, 21, 28, 71, 78, 82, 87, 22, 27, 72, 77, 0};

INT NEAR PASCAL minmax(BYTE b[max_depth + 2][100], INT move, INT friendly, INT enemy, INT ply, INT vmin, INT vmax);
...
VOID NEAR PASCAL drawboard(BYTE b[max_depth+2][BoardSize])
{
    register INT i;
    INT lcx,lcy;
...
VOID NEAR PASCAL FlashMessageTop(HWND hWindow)
{
    flashtimes = 0;
    count = 4;
...
VOID NEAR PASCAL RevMessage(HWND hWindow, HDC hDC, register CHAR *pS, INT n, CHAR *pchPostStr)
{
    register CHAR *pch;
...
BOOL NEAR PASCAL RevInit(HANDLE hInstance)
{
    ...
    INT COLOR = GetDeviceCaps(hdc, NUMCOLORS) > 2;
...
VOID NEAR PASCAL RevMouseClick(HWND hWnd, POINT point)
{
    INT move;
    INT Si, Sj;
    INT yLineExt = 8 * yExt;
...
VOID NEAR PASCAL Previous(register INT *px, register INT *py)
{
    (*px)--;
    if (*px < 0)
    {
...
VOID NEAR PASCAL ShowNextMove(HWND hwnd, BOOL fforward)
{
    INT x, y;
    INT potentialmove;
    BOOL done;
...
VOID NEAR PASCAL RevChar(HWND hwnd, register WORD code)
{
    INT a;
    POINT curpoint;
...
VOID APIENTRY InverseMessage(register HWND hWindow, UINT message, UINT_PTR wParam, DWORD lParam)
{
    HDC hDC;
...
LRESULT APIENTRY ReversiWndProc(HWND hWnd, register UINT message, WPARAM wParam, LPARAM lParam)
{
    HMENU hm;
    PAINTSTRUCT ps;
    POINT curpoint;
